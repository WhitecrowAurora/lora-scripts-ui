...
const S_SPEED_SDXL = [
  ...
  { key: 'experimental_attention_profile_window', type: 'number', label: '统计窗口 (步)', desc: '每 N 个优化步输出一次聚合耗时摘要', defaultValue: 50, min: 1, visibleWhen: when('experimental_attention_profile_enabled', true) },
...
const S_PEAK_VRAM = [
  ...
  { key: 'peak_vram_auto_protection_enabled', type: 'boolean', label: '动态显存自动保护', desc: '遇到真实 OOM 时自动降一档保护并重试当前 step，稳定后逐档恢复。注意：开启后训练速度会略有下降，仅建议在显存紧张时使用', defaultValue: false, visibleWhen: when('peak_vram_control_enabled', true) },
];
