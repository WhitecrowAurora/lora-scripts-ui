...
export function createDatasetPageController({ api, state, renderView, showToast }) {
  ...
  async function runTagger() {
    ...
    const params = {
      path: pathVal,
      interrogator_model: $('#tagger-model')?.value || 'wd-convnext-v3',
  ...
  function renderCaptionCleanup() {
    ...
    content.innerHTML = `
      <section class="form-section">
        ...
        <div id="cleanup-job" style="margin-top:12px;font-size:0.9rem;color:var(--text-dim);"></div>
  ...
  function bindGlobals(targetWindow) {
    ...
    targetWindow.previewDatasetAnalysis = previewDatasetAnalysis;
    targetWindow.startDatasetAnalysis = startDatasetAnalysis;
    targetWindow.loadCachedDatasetAnalysis = loadCachedDatasetAnalysis;
    targetWindow.cancelDatasetAnalysisJob = cancelDatasetAnalysisJob;
    targetWindow.viewReviewQueue = viewReviewQueue;
    targetWindow.inspectFindingImage = inspectFindingImage;
    targetWindow.sendFindingToSuggestions = sendFindingToSuggestions;
    targetWindow.loadTagSuggestions = loadTagSuggestions;
    targetWindow.refreshTagSuggestionsIndex = refreshTagSuggestionsIndex;
    targetWindow.refineTagSuggestionsWithLlm = refineTagSuggestionsWithLlm;
    targetWindow.useSuggestionPreview = useSuggestionPreview;
    targetWindow.runCaptionCleanupPreview = runCaptionCleanupPreview;
    targetWindow.runCaptionCleanupApply = runCaptionCleanupApply;
    targetWindow.cancelCleanupJob = cancelCleanupJob;
