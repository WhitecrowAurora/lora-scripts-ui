...
import {
  ...
  gradientCheckpointGatedResidencyOptions,
...
// 可以给多个指标名：几个指标共用同一个模型时（DINO 编码器同时被 dino_i 和
// copy_risk 使用），只挂在其中一个上会让另一个指标单独勾选时看不到模型路径 ——
// 而它恰好正需要这些路径，结果就是指标报「模型不可用」，而用户在界面上找不到
// 任何可填的地方。
const qualityMetricEnabled = (...names) => (config) => {
  const raw = config.quality_evaluation_metrics;
  const metrics = Array.isArray(raw) ? raw : String(raw || '').split(/[;,]/);
  const wanted = new Set(names);
  return Boolean(config.quality_evaluation_enabled)
    && metrics.some((item) => wanted.has(String(item).trim().toLowerCase().replaceAll('-', '_')));
};
...
export const S_KEEP_W8_TUNEQDM = [
  ...
  { key: 'layer_precision_policy', type: 'select', label: '分层精度策略', title: 'layer_precision_policy', desc: 'sensitive_bf16 会保护输入/输出、时间调制、AdaLN/Norm 等敏感层；导出与 keep-W8 共用同一策略。', defaultValue: 'sensitive_bf16', options: [
    { value: 'sensitive_bf16', label: '敏感层 BF16（推荐）' },
    { value: 'off', label: '关闭（旧版一刀切）' }
  ] },
  { key: 'layer_precision_default', type: 'select', label: '普通层默认精度', title: 'layer_precision_default', desc: '未被敏感规则或显式覆盖命中的量化候选层使用此精度。', defaultValue: 'int8_convrot', options: [
    { value: 'int8_convrot', label: 'ConvRot INT8' },
    { value: 'int8_rowwise', label: 'Rowwise INT8' },
    { value: 'fp8_scaled', label: 'Scaled FP8' },
    { value: 'bf16', label: 'BF16' }
  ] },
  { key: 'layer_precision_sensitivity_mode', type: 'select', label: '层敏感度评估', title: 'layer_precision_sensitivity_mode', desc: 'off 使用静态策略；weight 做有界权重重建误差扫描；activation_geometry 使用已有聚合 E[x²]，不保存原始激活。', defaultValue: 'off', options: [
    { value: 'off', label: '关闭' },
    { value: 'weight', label: '权重重建误差' },
    { value: 'activation_geometry', label: '激活几何 E[x²]' }
  ] },
  { key: 'layer_precision_activation_geometry_path', type: 'file', pickerType: 'model-file', label: '激活几何 Artifact', title: 'layer_precision_activation_geometry_path', desc: '包含 lulynx activation-geometry trace 的 safetensors；只读取聚合 E[x²]。', defaultValue: '', visibleWhen: when('layer_precision_sensitivity_mode', 'activation_geometry') },
  { key: 'layer_precision_rules_json', type: 'textarea', label: '分层精度规则 JSON', title: 'layer_precision_rules_json', desc: '可选：按 family/glob/block 指定 FP8、INT8 或 BF16；普通规则不能覆盖内置敏感层。', defaultValue: '', visibleWhen: when('performance_expert_mode', true) },
  { key: 'layer_precision_overrides_json', type: 'textarea', label: '分层精度强制覆盖 JSON', title: 'layer_precision_overrides_json', desc: '高级显式覆盖，可用于经过 A/B 签字的精确层分配。', defaultValue: '', visibleWhen: when('performance_expert_mode', true) },
  { key: 'quant_requantize_policy', type: 'select', label: '量化模型再次量化', title: 'quant_requantize_policy', desc: 'avoid 检测到已有 INTx/FPx pack 时退避，避免二次量化噪声；allow 仅用于受控 A/B。', defaultValue: 'avoid', options: [
    { value: 'avoid', label: '退避（推荐）' },
    { value: 'allow', label: '允许二次量化' }
  ] },
...
export const S_TURBOCORE_DATA_PIPELINE = [
  { key: 'turbocore_data_pipeline_enabled', type: 'boolean', label: 'TurboCore 原生数据管线', title: 'turbocore_data_pipeline_enabled', desc: 'SDXL/SD1.5 可选 Rust/native 图像解码与预取路径；遇到不兼容的数据集行为会回退现有 Python DataLoader。', defaultValue: false },
  { key: 'turbocore_prefetch_depth', type: 'number', label: 'TurboCore 预取深度', title: 'turbocore_prefetch_depth', desc: '原生数据管线的预取队列深度。仅在启用 TurboCore 原生数据管线时生效。', defaultValue: 2, min: 1, step: 1, visibleWhen: when('turbocore_data_pipeline_enabled', true) },
];
...
// 开着模块级 Offload 时，两个编译后端不可选（后端 module_offload_contract 会拒）。
// 只禁这两项、保留 optimized / eager，用户才有化解冲突的出路；整个下拉框不置灰。
const COMPILE_BACKEND_VALUES = new Set(['thunder', 'torch_compile']);
export const executionBackendOptionsFor = (config = {}) => {
  if (!config.module_offload_enabled) return EXECUTION_BACKEND_OPTIONS;
  return EXECUTION_BACKEND_OPTIONS.map((option) => (
    COMPILE_BACKEND_VALUES.has(option.value)
      ? { ...option, disabled: true, disabledReason: '与「模块级 Offload」互斥，请先关闭模块级 Offload' }
      : option
  ));
};
...
export const S_EXECUTION_BACKEND = [
  { key: 'execution_backend', type: 'select', label: '训练执行后端', title: 'execution_backend', desc: '默认优化运行时不做图编译；需要编译时优先选择 Thunder。开启模块级 Offload 时两个编译后端不可用。', defaultValue: 'optimized', options: executionBackendOptionsFor },
...
// VAE 分块。单独成组：除了「内存与 Offload」，anima 各训练类型的「速度优化」页也要用它。
export const S_VAE_TILING = [
  { key: 'vae_tiling', type: 'boolean', label: 'VAE 分块', desc: 'VAE 分块解码。anima 预览实测：解码峰值 2123 → 1033 MB（省 2.05×），代价是 1.16× 解码时间。显存紧张时建议打开。', defaultValue: false }
];

// vae_slicing 的默认值必须与后端保持 true。它按 batch 维切开逐张过 VAE，所以 batch=1 时
// 是彻底的 no-op（实测峰值逐 MiB 相同），batch>1 才省显存；4070 Ti SUPER / 1024px / bf16
// 实测 batch=4 编码峰值 6343 → 1738 MiB、解码 8391 → 2650 MiB，代价是编码 +4.5%（解码反而
// 更快，大块分配的压力没了）。batch=8 不开时解码峰值 16609 MiB 越过 16 GiB 卡会走驱动换页。
export const S_VAE_MEMORY = [
  { key: 'vae_slicing', type: 'boolean', label: 'VAE 切片', desc: 'VAE 编码/解码时按 batch 切开逐张处理（省显存）。batch=1 是位级无操作；batch=4 干净卡实测省 4.6～5.7 GiB 峰值，编码略慢、解码稳定快约两成，合计净快约 220ms/批。', defaultValue: true },
  ...S_VAE_TILING
];
...
export const S_DIT_PERFORMANCE_EXPERT = [
  { key: 'performance_expert_mode', type: 'boolean', label: '性能专家模式', title: 'performance_expert_mode', desc: '启用高级性能选项（TurboCore 优化器加速、Native 优化器等）。', defaultValue: false },
...
export const S_OPTIMIZER_STATE_PAGING = [
  ...
  { key: 'blockwise_fused_optimizers', type: 'boolean', label: '优化器状态分块分页', title: 'blockwise_fused_optimizers', desc: '把优化器状态分页的粒度降到每次一个 block：step 期只有当前 block 的 optimizer state 回到参数设备，峰值从全模型 state 降到单个 block（8 block 骨干实测 12.5%），数学逐位不变（分批靠临时置空其余梯度，不换优化器）。需先开「优化器状态分页」且驻留设备为 CPU；跨 param group 归约的优化器（如 Prodigy）会自动退回整份换入。', defaultValue: false, visibleWhen: when('optimizer_state_paging_enabled', true) }
];
...
export const ANIMA_BLOCK_RESIDENCY_FIELDS = [
  ...
  { key: 'anima_block_offload_enabled', type: 'boolean', label: 'Anima Block LRU Offload', title: 'anima_block_offload_enabled', desc: '按 block 的 LRU 换出：只保留活跃 block 在显存，其余换到内存；与 anima_block_residency=streaming_offload 互斥。', defaultValue: false, visibleWhen: nonResidentBlockMode('anima_block_residency') },
  { key: 'anima_block_offload_gpu_slots', type: 'number', label: 'LRU GPU 槽位数', title: 'anima_block_offload_gpu_slots', desc: '同时保留在 GPU 上的 block 数（当前 block 加最近用过的若干个，按 LRU 淘汰）。', defaultValue: 3, min: 1, max: 16, step: 1, visibleWhen: all(nonResidentBlockMode('anima_block_residency'), when('anima_block_offload_enabled', true)) },
...
export const FLUX2_OFFLOAD_FIELDS = [
  { key: 'flux2_block_residency', type: 'select', label: 'FLUX.2 Block 驻留', title: 'flux2_block_residency', desc: 'FLUX.2 Block 驻留。block_offload 需要梯度检查点配对才真省显存。', defaultValue: 'block_offload', options: gradientCheckpointGatedResidencyOptions([
    { value: 'resident', label: 'resident（全驻留 GPU）' },
    { value: 'block_offload', label: 'block_offload（自适应卸载）' }
  ])},
...
export const ZIMAGE_OFFLOAD_FIELDS = [
  { key: 'zimage_block_residency', type: 'select', label: 'Z-Image Block 驻留', title: 'zimage_block_residency', desc: 'Z-Image Block 驻留。block_offload 需要梯度检查点配对才真省显存。', defaultValue: 'block_offload', options: gradientCheckpointGatedResidencyOptions([
    { value: 'resident', label: 'resident（全驻留 GPU）' },
    { value: 'block_offload', label: 'block_offload（自适应卸载）' }
  ])},
...
export const WAN22_OFFLOAD_FIELDS = [
  { key: 'wan22_block_residency', type: 'select', label: 'Wan2.2 Block 驻留', title: 'wan22_block_residency', desc: 'Wan2.2 Block 驻留。block_offload 需要梯度检查点配对才真省显存。', defaultValue: 'block_offload', options: gradientCheckpointGatedResidencyOptions([
...
export const LTX23_OFFLOAD_FIELDS = [
  { key: 'ltx23_block_residency', type: 'select', label: 'LTX-2.x Block 驻留', title: 'ltx23_block_residency', desc: '22B 建议 block_offload；block_offload 需要梯度检查点配对才真省显存。', defaultValue: 'block_offload', options: gradientCheckpointGatedResidencyOptions([
    { value: 'resident', label: 'resident' },
    { value: 'block_offload', label: 'block_offload' }
  ])},
...
export const BOOGU_OFFLOAD_FIELDS = [
  { key: 'boogu_block_residency', type: 'select', label: 'Boogu Block 驻留', title: 'boogu_block_residency', desc: '默认 resident（Layer Offload 未实现）；block_offload 需要梯度检查点配对才真省显存。', defaultValue: 'resident', options: gradientCheckpointGatedResidencyOptions([
    { value: 'resident', label: 'resident（全驻留 GPU）' },
...
export const S_CACHED_DATALOADER = [
  ...
  { key: 'cached_dataloader_pin_memory', type: 'select', label: '缓存 pin_memory', title: 'cached_dataloader_pin_memory', desc: 'auto / true / false。', defaultValue: 'auto', options: [
    { value: 'auto', label: '自动（有 CUDA 就开）' },
    { value: 'true', label: '强制开启' },
    { value: 'false', label: '强制关闭' },
  ], visibleWhen: when('cached_dataloader_auto_policy', false) }
];
...
// 首轮缓存构建的编码批次旋钮（与 newbie 的 newbie_cache_build_* 一对一对应）。
// 后端读点:trainer_anima_cache_runtime.py 的 _build_anima_cache_now。
export const S_ANIMA_CACHE_BUILD = [
  { key: 'anima_cache_build_batch_size', type: 'number', label: '缓存构建批大小', title: 'anima_cache_build_batch_size', desc: '首轮缓存构建时每批 VAE 编码的图像数。越大越快、显存占用越高（VAE 切片已默认打开，按批维逐张编码，数值与批大小无关）。', defaultValue: 8, min: 1, step: 1 },
  { key: 'anima_cache_build_prefetch', type: 'boolean', label: '缓存构建 CPU 预取', title: 'anima_cache_build_prefetch', desc: '首轮缓存构建时在 CPU 线程预解码下一批图像，与 GPU 编码重叠。', defaultValue: false }
];
...
export const S_SAVE = [
  ...
  { key: 'export_comfy_int8_groupsize', type: 'number', label: 'ConvRot Group Size', title: 'export_comfy_int8_groupsize', desc: '必须是 4 的幂；默认 256。非法值会拒绝导出，不再静默伪装 ConvRot。', defaultValue: 256, min: 4, step: 4, visibleWhen: when('export_comfy_int8_base', true) },
  ...
  { key: 'save_every_n_epochs', type: 'number', label: '每 N 轮保存', title: 'save_every_n_epochs', desc: '每 N epoch（轮）自动保存一次模型。0 表示不按轮保存，此时「每 N 步保存」必须大于 0——两者不能同时为 0，否则整个训练只在正常结束时落盘一次，中途中断拿不到权重', defaultValue: 1, min: 0 },
  { key: 'save_every_n_steps', type: 'number', label: '每 N 步保存', title: 'save_every_n_steps', desc: '每 N 步自动保存一次模型。0 表示不按步保存，此时「每 N 轮保存」必须大于 0。两者可以同时开，但间隔都设得很小会写出大量检查点', defaultValue: '', min: 0 },
...
export const S_CAPTION = [
  ...
  { key: 'caption_dropout_rate', type: 'number', label: '全部标签丢弃概率', title: 'caption_dropout_rate', desc: '全部标签丢弃概率', defaultValue: '', min: 0, max: 1, step: 0.01 },
  ...
  { key: 'caption_tag_dropout_rate', type: 'number', label: '按标签丢弃概率', title: 'caption_tag_dropout_rate', desc: '按逗号分隔的标签来随机丢弃 tag 的概率', defaultValue: '', min: 0, max: 1, step: 0.01 },
  ...
  { key: 'caption_source_trigger_tokens', type: 'textarea', label: '触发词列表', title: 'caption_source_trigger_tokens', desc: '逗号或换行分隔；留空时优先尝试 JSON 中的 concept / concept_group / identity / character 字段。', defaultValue: '', visibleWhen: (c) => (String(c.model_train_type || '').includes('anima') || String(c.model_train_type || '').includes('newbie')) && c.caption_source_mix_enabled === true },
...
export const S_LR = [
  ...
  { key: 'optimizer_type', type: 'select', label: '优化器', title: 'optimizer_type', desc: '优化器设置。pytorch_optimizer.<名字> 会转成 PytorchOptimizer 并把 name= 写进 optimizer_args，需要装 pytorch-optimizer 插件。', defaultValue: 'AdamW8bit', options: ALL_OPTIMIZERS },
  // Frontier 双开关：已并入 optimizer_type，主路径隐藏（键保留兼容旧配置/API）
  ...
  { key: 'lora_plus_lr_ratio', type: 'number', label: 'LoRA+ LR 倍率', title: 'lora_plus_lr_ratio', desc: 'B 相对 A 的学习率倍率（常用 16）。', defaultValue: 16.0, min: 1, step: 0.1, visibleWhen: when('lora_plus_enabled', true) },
...
export const S_PREVIEW = [
  ...
  { key: 'preview_device', type: 'select', label: '预览设备', title: 'preview_device', desc: '混合档=DiT 留在 GPU，VAE 与文本编码器放 CPU，出预览图时显存不再波动（native anima 与 newbie 支持，其他族按 GPU 处理）；GPU 档最快，但 fp32 整帧解码约 8.5GB，配置顶到上限的机器常在第一张预览就崩；CPU 档整张图在独立进程渲染，训练不暂停，但一张 512 图约 6 分钟', defaultValue: 'hybrid', options: ['hybrid', 'gpu', 'cpu', 'off'], visibleWhen: when('enable_preview', true) },
  { key: 'preview_component_residency', type: 'select', label: '预览组件常驻', title: 'preview_component_residency', desc: 'free=出图后释放预览借用的 VAE/文本编码器，把约 1.4GB 还给训练；keep=一直留着，省掉每次重新读盘。只会释放预览自己加载的那一份，训练本身已有的组件不动', defaultValue: 'free', options: ['free', 'keep'], visibleWhen: all(when('enable_preview', true), oneOf('preview_device', ['hybrid', 'gpu'])) },
  ...
  { key: 'sample_prompts', type: 'textarea', label: '采样提示词列表（sample_prompts）', title: 'sample_prompts', desc: '后端 sample_prompts：每行一个', defaultValue: '', visibleWhen: when('enable_preview', true) },
  ...
  { key: 'sample_width', type: 'number', label: '预览图宽度', title: 'sample_width', desc: '预览图宽；0 = 沿用训练素材分辨率（resolution 的宽）。填具体数值则固定用该尺寸。', defaultValue: 0, min: 0, visibleWhen: when('enable_preview', true) },
  { key: 'sample_height', type: 'number', label: '预览图高度', title: 'sample_height', desc: '预览图高；0 = 沿用训练素材分辨率（resolution 的高）。宽高各自独立，可只钉住一边。', defaultValue: 0, min: 0, visibleWhen: when('enable_preview', true) },
  ...
  { key: 'sample_scheduler', type: 'string', label: '采样调度器覆盖（sample_scheduler）', title: 'sample_scheduler', desc: '后端目前未消费该字段（仅随请求透传），预览采样器由上面的 sample_sampler 决定。', defaultValue: '', visibleWhen: when('enable_preview', true) }
];

// rectified-flow DiT 族（anima / newbie）的预览组变体。
// 两处差异，都是实测出来的：
//   · CFG 出厂 4 而非 7 —— 这些族在 cfg≥5 就把预览解码成近黑帧
//     （实测均值亮度 cfg1=0.503 / cfg4=0.225 / cfg5=0.094 / cfg7.5=0.024）。
//   · 多两个求解器旋钮 —— ER-SDE 只有这两族有，SDXL/flux 线不该出现。
const FLOW_PREVIEW_META = {
  sample_cfg: {
    defaultValue: 4,
    max: 12,
    desc: 'CFG Scale。该族建议 3.5~4；≥5 会让预览越来越暗直至全黑。',
  },
};

export const S_PREVIEW_FLOW = [
  ...remapFieldMeta(S_PREVIEW, FLOW_PREVIEW_META),
  { key: 'sample_algorithm', type: 'select', label: '预览求解器', title: 'sample_algorithm', desc: 'sde = ER-SDE-Solver-3（随机，细节更丰富）；ode = 确定性 Euler（同种子可复现）。', defaultValue: 'sde', options: [
    { value: 'sde', label: 'ER-SDE（随机）' },
    { value: 'ode', label: 'Euler ODE（确定性）' },
  ], visibleWhen: when('enable_preview', true) },
  { key: 'sample_sde_eta', type: 'number', label: 'ER-SDE 随机强度 eta', title: 'sample_sde_eta', desc: '随机项强度，出厂 1.0 就是预期档。0 会转去确定性 Euler（与 ode 逐字节相同）；0~1 之间能出图，但会按比例欠注入，背景容易塌成平涂。', defaultValue: 1.0, min: 0, max: 1, step: 0.05, visibleWhen: all(when('enable_preview', true), when('sample_algorithm', 'sde')) },
];

export const S_QUALITY_EVAL = [
  { key: 'quality_evaluation_enabled', type: 'boolean', label: '训练结束质量评估', title: 'quality_evaluation_enabled', desc: '训练完成后计算配置的图像质量指标。', defaultValue: false },
  ...
  { key: 'quality_evaluation_metrics', type: 'string', label: '评估指标', title: 'quality_evaluation_metrics', desc: '逗号分隔：fid,clip_score,lpips,dino_i,copy_risk,wd14_tag_recall。copy_risk 拿生成图去比「训练图」找最相近的一张，其他指标都是比没训过的留出图 —— 只有它能把「学会了主体」和「把训练图背下来了」分开，因为这两种情况在所有相似度指标上都是同一个走向（都在涨）。', defaultValue: 'fid,clip_score', visibleWhen: when('quality_evaluation_enabled', true) },
  ...
  { key: 'quality_evaluation_reference_source', type: 'select', label: '参考图来源', title: 'quality_evaluation_reference_source', desc: 'FID / LPIPS / DINO-I / 标签召回拿哪批真图当对照。holdout = 本次训练留出、没被训过的图（默认）；手动目录 = 用下面填的目录。对照图若是训练过的图，这四个指标就变成「越照抄训练图分越高」，且不报错、分数还更好看。', defaultValue: 'holdout', options: [
    { value: 'holdout', label: '训练留出集（推荐）' },
    { value: 'manual_dir', label: '手动指定目录' },
  ], visibleWhen: when('quality_evaluation_enabled', true) },
  { key: 'fid_real_image_dir', type: 'folder', pickerType: 'folder', label: '指标参考图目录', title: 'fid_real_image_dir', desc: '仅在「参考图来源 = 手动指定目录」时生效。别填训练集目录：那等于让指标奖励背图。', defaultValue: '', visibleWhen: all(when('quality_evaluation_enabled', true), when('quality_evaluation_reference_source', 'manual_dir')) },
  { key: 'quality_dino_model', type: 'string', label: 'DINO 模型名', title: 'quality_dino_model', desc: '本地 DINO hub 入口名。', defaultValue: 'dinov2_vits14', visibleWhen: qualityMetricEnabled('dino_i', 'copy_risk') },
  { key: 'quality_dino_code_path', type: 'folder', pickerType: 'folder', label: 'DINO 本地代码目录', title: 'quality_dino_code_path', desc: '必须包含 hubconf.py；评估只执行该本地入口。', defaultValue: '', visibleWhen: qualityMetricEnabled('dino_i', 'copy_risk') },
  { key: 'quality_dino_model_path', type: 'file', pickerType: 'file', label: 'DINO 本地权重文件', title: 'quality_dino_model_path', desc: '必须选择已有权重文件；评估不会联网或自动下载。', defaultValue: '', visibleWhen: qualityMetricEnabled('dino_i', 'copy_risk') },
  { key: 'quality_dino_device', type: 'select', label: 'DINO 评估设备', title: 'quality_dino_device', desc: 'CPU 更省显存；auto 跟随训练设备。', defaultValue: 'cpu', options: ['cpu', 'auto', 'cuda'], visibleWhen: qualityMetricEnabled('dino_i', 'copy_risk') },
  { key: 'quality_copy_risk_threshold', type: 'number', label: '复制风险阈值', title: 'quality_copy_risk_threshold', desc: '超过这个相似度的样本会被单独计数，提示你打开图和它的最近邻训练图对照着看。这是「值得看一眼」的门槛，不是「判定为抄图」的门槛 —— 相似度到多少才算复制没有可靠标准，所以这里不做判决、不停训练。真正有信息量的是走势：留出集保真度没动而复制风险一路在涨，就是开始背图了。', defaultValue: 0.95, min: 0, max: 1, step: 0.01, visibleWhen: qualityMetricEnabled('copy_risk') },
  { key: 'quality_copy_risk_max_candidates', type: 'number', label: '复制风险比对图上限', title: 'quality_copy_risk_max_candidates', desc: '最近邻要拿每张生成图去比训练图，训练集大的时候这一步能比训练本身还久，所以设了上限（在排序后均匀取样，不是只取前面几张）。-1 = 不限，愿意等就填它。', defaultValue: 512, min: -1, step: 1, visibleWhen: qualityMetricEnabled('copy_risk') },
  { key: 'quality_wd14_model', type: 'select', label: '标签召回模型', title: 'quality_wd14_model', desc: '只读取本地缓存中的模型与词表。', defaultValue: 'wd-eva02-large-v3', options: ['wd-eva02-large-v3', 'wd-convnext-v3', 'wd-swinv2-v3', 'wd-vit-v3'], visibleWhen: qualityMetricEnabled('wd14_tag_recall') },
  { key: 'quality_wd14_threshold', type: 'number', label: '标签置信度阈值', title: 'quality_wd14_threshold', desc: '生成图与参考图使用相同阈值。', defaultValue: 0.35, min: 0, max: 1, step: 0.01, visibleWhen: qualityMetricEnabled('wd14_tag_recall') },
  ...
  { key: 'preference_models', type: 'ordered_multiselect', label: '偏好评分模型', title: 'preference_models', desc: '勾选要启用的评分模型，可拖动调整顺序。空=仅 pickscore。模型缺失时记 skipped，不静默当 0 分成功。', defaultValue: 'pickscore', visibleWhen: when('preference_scoring_enabled', true) }
];
...
// SD3 MMDiT transformer block residency. The runtime is wired, but physical
// 24GB hardware remains unsigned; managed VRAM profiles own these values.
export const SD3_OFFLOAD_FIELDS = [
  { key: 'sd3_block_residency', type: 'select', label: 'SD3 Block 驻留', title: 'sd3_block_residency', desc: 'SD3 MMDiT block 驻留模式。block_offload 需要梯度检查点配对才真省显存。', defaultValue: 'block_offload', options: gradientCheckpointGatedResidencyOptions([
    { value: 'resident', label: 'resident（全驻留 GPU）' },
    { value: 'block_offload', label: 'block_offload（按 block 卸载）' }
  ]) },
  { key: 'sd3_block_offload_min_param_mb', type: 'number', label: 'Block Offload 最小参数 MB', title: 'sd3_block_offload_min_param_mb', desc: '小于该体积的 block 不 offload。', defaultValue: 50.0, min: 0, step: 1, visibleWhen: when('sd3_block_residency', 'block_offload') },
  { key: 'sd3_block_offload_gpu_slots', type: 'number', label: 'Block Offload GPU 槽位', title: 'sd3_block_offload_gpu_slots', desc: '同时保留在 GPU 的 SD3 block 数。', defaultValue: 2, min: 1, max: 32, step: 1, visibleWhen: when('sd3_block_residency', 'block_offload') },
  { key: 'sd3_block_offload_prefetch_depth', type: 'number', label: 'Block Offload 预取深度', title: 'sd3_block_offload_prefetch_depth', desc: '异步预取后续 SD3 block 数。', defaultValue: 1, min: 0, max: 8, step: 1, visibleWhen: when('sd3_block_residency', 'block_offload') },
  { key: 'sd3_block_offload_pin_memory', type: 'boolean', label: 'Block Offload Pin Memory', title: 'sd3_block_offload_pin_memory', desc: 'CPU 侧 pinned 缓冲；24GB 未签字前默认关闭。', defaultValue: false, visibleWhen: when('sd3_block_residency', 'block_offload') },
  { key: 'sd3_block_offload_ratio', type: 'number', label: 'Block Offload 比例 %', title: 'sd3_block_offload_ratio', desc: '参与 block offload 的比例（0–100）。', defaultValue: 100, min: 0, max: 100, step: 1, visibleWhen: when('sd3_block_residency', 'block_offload') }
];

const TRAINING_VRAM_TIER_GB = [6, 8, 12, 16, 24, 32];
const TRAINING_VRAM_MINIMUM_BY_FAMILY = {
  anima: 6, krea2: 12, minimax_h3: 8, flux2: 16, sd3: 24,
  boogu: 16, zimage: 16, wan22: 6, ltx23: 6,
};
...
export function trainingVramProfileOptions(config = {}) {
  const family = trainingVramFamily(config);
  const minimum = TRAINING_VRAM_MINIMUM_BY_FAMILY[family];
  const unavailable = !minimum;
  const reason = unavailable
    ? '该模型尚未接入训练显存档位'
    : `该模型当前最低从 ${minimum} GB 开始，低档没有可信训练证据`;
  return [
    { value: 'off', label: '关闭' },
    { value: 'auto', label: '自动', disabled: unavailable, disabledReason: reason },
    ...TRAINING_VRAM_TIER_GB.map((size) => ({
      value: `${size}g`,
      label: `${size} GB${minimum && size === minimum ? '（当前最低）' : ''}`,
      disabled: unavailable || size < minimum,
...
export const TRAINING_VRAM_PROFILE_CONTROL_FIELDS = [
...
export const S_SPEED_SDXL = [
  ...
  { key: 'mem_eff_attn', type: 'boolean', label: '低显存注意力', title: 'mem_eff_attn', desc: '高级覆盖：省显存 attention（比 xformers 更省显存但更慢）。', defaultValue: false, visibleWhen: when('performance_expert_mode', true) },
  ...
  { key: 'te_vae_offload_strategy', type: 'select', label: 'TE/VAE Offload 策略', title: 'te_vae_offload_strategy', desc: 'phase 为默认训练生命周期策略;grouped 让 CLIP-L 与 VAE 常驻显卡、T5 仍每步搬运', defaultValue: 'phase', options: ['phase', 'aggressive', 'resident', 'grouped'] },
  ...
  { key: 'vram_swap_to_ram', type: 'boolean', label: 'VRAM Swap to RAM', title: 'vram_swap_to_ram', desc: '让原生 LoRA / LoRA-FA / lulynx 渐进秩 LoRA / VeRA', defaultValue: false },
...
export const S_SPEED_FLOW = [
  ...
  { key: 'weight_compression_preset', type: 'select', label: '权重压缩预设', title: 'weight_compression_preset', desc: '冻结基座压缩。推荐「骨干 INT8」。与 keep_w8 / fp8_base 互斥；下方「训练量化捷径」不为 off 时以捷径为准，本项被覆盖。', defaultValue: 'off', options: [
    { value: 'off', label: '关闭' },
  ...
  { key: 'weight_compression_format', type: 'select', label: '压缩格式', title: 'weight_compression_format', desc: '底层压缩后端。fp8_e4m3 默认；fp8_scaled_e4m3 带 per-output-channel 缩放,仅 krea2 / FLUX.1 / FLUX.2 / Z-Image / Wan2.2 支持且压缩目标必须是 backbone。', defaultValue: 'fp8_e4m3', options: [
    { value: 'fp8_e4m3', label: 'fp8_e4m3' },
    { value: 'fp8_scaled_e4m3', label: 'fp8_scaled_e4m3（krea2 / FLUX.1 / FLUX.2 / Z-Image / Wan2.2，仅 backbone）' },
...
export const S_DISTRIBUTED = [
  ...
  { key: 'nccl_proto', type: 'select', label: 'NCCL 协议', title: 'nccl_proto', options: ['', 'LL', 'LL128', 'Simple'], desc: 'NCCL 集合通信协议（NCCL_PROTO）。留空=自动选择。影响本进程所有 NCCL 通信（不只 TP）；无通用“必然更快”结论，V100/PCIe 建议 LL128 与 Simple 实测对比', defaultValue: '', visibleWhen: when('enable_distributed_training', true) },
  ...
  { key: 'ddp_gradient_as_bucket_view', type: 'boolean', label: 'DDP Bucket View', defaultValue: true, visibleWhen: when('enable_distributed_training', true) },
...
export const S_LULYNX_SDXL = [
  ...
  { key: 'lulynx_block_weight_enabled', type: 'boolean', label: '启用 BlockWeight (SDXL)', title: 'lulynx_block_weight_enabled', desc: '按 SDXL U-Net 结构分组配权：IN（编码侧 0-11）/ MID / OUT（解码侧 0-11），另含 Text Encoder。', defaultValue: false, visibleWhen: when('lulynx_experimental_core_enabled', true) },
...
export const S_SAFEGUARD = [
  ...
  { key: 'safeguard_safe_state_interval', type: 'number', label: '安全快照间隔', title: 'safeguard_safe_state_interval', desc: '每 N 个优化 step 保存一次可回滚快照。', defaultValue: 100, min: 1, visibleWhen: when('safeguard_enabled', true) },
  { key: 'safeguard_nan_check_interval', type: 'number', label: 'NaN 检查间隔', title: 'safeguard_nan_check_interval', desc: '每 N 个优化 step 检查一次 NaN / Inf loss', defaultValue: 10, min: 1, visibleWhen: when('safeguard_enabled', true) },
  ...
  { key: 'safeguard_loss_spike_threshold', type: 'number', label: 'Loss Spike 阈值', title: 'safeguard_loss_spike_threshold', desc: '当前 loss 超过滚动平均值多少倍时，判定为 spike 并跳过该 step', defaultValue: 10.0, min: 1, step: 0.1, visibleWhen: when('safeguard_enabled', true) },
  { key: 'safeguard_loss_window_size', type: 'number', label: 'Loss 窗口大小', title: 'safeguard_loss_window_size', desc: '用于判定 loss spike 的滚动窗口大小', defaultValue: 50, min: 2, visibleWhen: when('safeguard_enabled', true) },
...
// 注意力后端下拉:S_ADV 与 anima 自己的系统分组共用同一份声明。分头写两份会漂,
// 而这个字段的取值集合必须与 attentionCapabilities 的别名表严格对齐。
export const ATTENTION_BACKEND_FIELD = { key: 'attention_backend', type: 'select', label: 'Attention 后端（高级）', title: 'attention_backend', desc: 'auto=按当前模型家族与执行环境解析；当前运行时装不出来的后端会置灰，仅显式支持的后端不会被 auto 自动选中', defaultValue: 'auto', attentionBackendOptions: true, options: [
...
// 系统设置段(执行环境 Profile / 指定显卡 / 自定义 TOML 覆盖)。这三个控件在 S_ADV 里不相邻
// ——exec 在 attention 之前、TOML 在十几行之后——所以没法从 S_ADV 里 spread 出来,只能逐字复制
// 成独立常量;S_ADV 本体一个字不动,免得动到已经露着它们的那 30 个训练类型的字段顺序。
// 给的是「本来就缺这一段」的训练类型用:exec 决定跑在哪个 runtime,gpu_ids 决定用哪几张卡,
// 少了它们的类型只能继承 launcher 启动时的选择,改不了前提。
// gpu_ids 的控件类型逐分叉不同,本分叉是 ordered_multiselect(带 emptyHint) —— 照抄本分叉的字面量。
export const S_SYSTEM_ENV = [
  ...
  { key: 'gpu_ids', type: 'ordered_multiselect', deviceListOptions: true, label: '指定显卡', title: 'gpu_ids', desc: '勾选参与训练的显卡；值仍是逗号分隔的数字编号（如 0,1）。留空时继承 launcher 启动时选择的显卡。读不到显卡表时退回成手填。', defaultValue: '', emptyHint: '未选择显卡（继承 launcher 启动时的选择）' },
...
export const S_ADV = [
  ...
  { key: 'ema_update_after_step', type: 'number', label: 'EMA 起始步', title: 'ema_update_after_step', desc: '从第几个优化 step 开始更新 EMA', defaultValue: 100, min: 0, visibleWhen: when('ema_enabled', true) },
  ...S_SAFEGUARD,
  ...S_WAVELET_LOSS
];
...
export const S_WEIGHT_NOISE = [
  { key: 'lulynx_weight_noise_enabled', type: 'boolean', label: 'LoRA 权重噪声', title: 'lulynx_weight_noise_enabled', desc: '每个 optimizer.step 后对可训练 LoRA/适配器参数加零均值高斯扰动，不碰底模。', defaultValue: false },
  { key: 'lulynx_weight_noise_mode', type: 'select', label: '权重噪声模式', title: 'lulynx_weight_noise_mode', desc: 'relative（默认）：σ 按参数 RMS 缩放，零初始化 up 层早期几乎不加噪；absolute：全局固定 σ。', defaultValue: 'relative', options: [
...
export const S_NOISE = [
  { key: 'noise_offset', type: 'number', label: '噪声偏移', title: 'noise_offset', desc: '在训练中添加噪声偏移来改良生成非常暗或者非常亮的图像；0 表示关闭，启用时建议从 0.1 开始', defaultValue: '', min: 0, step: 0.01 },
  ...
  { key: 'perlin_noise_offset_enabled', type: 'boolean', label: 'Perlin 噪声偏移', title: 'perlin_noise_offset_enabled', desc: '用 Perlin 场替代均匀 noise_offset，偏移量按空间位置变化而不是全图同一个常数。', defaultValue: false },
...
export const S_THERMAL = [
  ...
  { key: 'gpu_duty_cycle', type: 'number', label: 'GPU 占空比', title: 'gpu_duty_cycle', desc: '每个优化 step 后按比例插入空闲降温，取值 0.2~1.0；留空或 1.0 = 不插入，越小空闲占比越大（温度更低、步时更长）。', defaultValue: '', min: 0.2, max: 1, step: 0.05 },
...
export const cnLR = [
  ...
  ...S_LOSS_AWARE_LR,
  // (separator for TypeScript parser)
  { key: 'optimizer_type', type: 'select', label: '优化器', title: 'optimizer_type', desc: '优化器。pytorch_optimizer.<名字> 会转成 PytorchOptimizer 并把 name= 写进 optimizer_args，需要装 pytorch-optimizer 插件。', defaultValue: 'AdamW8bit', options: ALL_OPTIMIZERS }
];
