...
// ---- 谓词组合器 ----
// 说明：这些谓词返回闭包，闭包捕获的 key 字面量在运行时 toString() 里看不到。
// 为了让 CONDITIONAL_KEYS 能从 schema 自动推导（见 schemaIndex.collectConditionalKeys），
// 每个组合器都把它依赖的 config key 挂到返回函数的 `.deps` 上（Set 去重）。
// 新增字段只要用这些组合器写 visibleWhen，就会被自动登记为联动重渲染键，无需手工维护清单。
function _tag(fn, deps) {
  fn.deps = deps instanceof Set ? deps : new Set(deps.filter(Boolean));
  return fn;
}
function _mergeDeps(fns) {
  const out = new Set();
  for (const f of fns) {
    if (f && f.deps) for (const k of f.deps) out.add(k);
  }
  return out;
}

export function when(key, expected) { return _tag((c) => c[key] === expected, [key]); }
export function all(...fns) { return _tag((c) => fns.every((f) => f(c)), _mergeDeps(fns)); }
export function oneOf(key, values) { return _tag((c) => values.includes(c[key]), [key]); }
export function optimizerIs(value) { return _tag((c) => String(c.optimizer_type || '').trim().toLowerCase() === String(value || '').trim().toLowerCase(), ['optimizer_type']); }
export function adamwFamilyOptimizer(c) { return ['adamw', 'adamw8bit'].includes(String(c.optimizer_type || '').trim().toLowerCase()); }
adamwFamilyOptimizer.deps = new Set(['optimizer_type']);
export function swapEnabled(c) { return c.swap_granularity && c.swap_granularity !== 'off'; }
swapEnabled.deps = new Set(['swap_granularity']);
export function nonResidentBlockMode(key) { return _tag((c) => c[key] && c[key] !== 'resident', [key]); }
export function streamingBlockMode(key) { return when(key, 'streaming_offload'); }
export function fieldValueIn(key, values) { return _tag((c) => values.includes(c[key]), [key]); }
...
export const nativeLoraInitSelected = _tag((c) => String(c.adapter_init_strategy || '').trim().toLowerCase() !== 'default', ['adapter_init_strategy']);
export const pissaInitSelected = _tag((c) => c.pissa_init === true || c.pissa_enabled === true || String(c.adapter_init_strategy || '').trim().toLowerCase() === 'pissa', ['pissa_init', 'pissa_enabled', 'adapter_init_strategy']);
...
export const nonLycorisNetworkSelected = _tag((c) => !LYCORIS_OR_OFT_NETWORK_MODULES.includes(c.network_module), ['network_module']);
