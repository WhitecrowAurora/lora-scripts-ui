...
window.inspectFindingImage = (encodedPath) => {
...
window.useSuggestionPreview = (index) => {
...
window.loadCachedDatasetAnalysis = async () => {
  ...
  try {
    ...
    if (data.status === 'stale') showToast('缓存结果已过期，建议重新分析。');
...
window.pollAnalysisJob = async (jobId, datasetPath) => {
  const jobEl = $('#analysis-job');
  const result = $('#analysis-result');
...
window.pollCleanupJob = async (jobId) => {
  const jobEl = $('#cleanup-job');
  const result = $('#cleanup-result');
  const timer = setInterval(async () => {
    try {
      ...
      if (data.status === 'completed') {
        ...
        if (result) result.innerHTML = `<div class="builtin-picker-empty"><span>清洗完成${changed != null ? `，预估改动 ${changed} 个文件` : ''}。</span></div>`;
...
window.cancelCleanupJob = async (jobId) => {
  try {
    await api.cancelJob(jobId);
    showToast('已请求取消清洗任务。');
  } catch (error) {
...
window.runTool = async (toolId, scriptName, keys) => {
  ...
  for (const key of keys) {
    ...
    const val = isCheckbox ? input.checked : input.value.trim();
    if (isCheckbox ? val : val) {
      if (!isCheckbox && listKeys.has(key)) {
